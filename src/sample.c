...
double gen_sample(struct continuouSignal *sig, double time) {
	double res = 0;
...
struct stream gen_samplePeriod(struct continuouSignal *sig, uint64_t sampleFreq) {
	struct stream res;
...
struct stream gen_sampleDuration(struct continuouSignal *sig, uint64_t sampleFreq, double duration){
	struct stream res;
	double samplePeriod;
    if(duration <= 0){
...
void gen_addComp(struct continuouSignal *sig, struct contSigComp *comp){
